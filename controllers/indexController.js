...
exports.clickedUser = AsyncError(async (req, res, next) => {
  ...
  const { posts, followers, following } = await User.findOne({ _id: id })
    .populate({
      path: "followers",
      populate: [{ path: "followers" }, { path: "following" }],
    })
...
/**
 * @api GET /userDataAPI/:type
 */
exports.userDataAPI = AsyncError(async (req, res, next) => {
  const { type } = req.params;

  const { posts, followers, following } = await User.findOne({
    _id: req.user._id,
  })
    .populate({
      path: "followers",
...
/**
 * @api GET /api/search/:type/:search
 */
    ...
    if (type === "user") {
      ...
      const post = await Post.find({
        $or: [
          ...
          {
            comments: {
              $elemMatch: { commentBody: { $regex: search, $options: "i" } },
            },
          },
        ],
      }).populate("owner");
...
exports.likes = AsyncError(async (req, res, next) => {
  ...
  if (
    likePost.likes.includes(req.user._id) &&
    user.fav.includes(req.params.id)
  ) {
      ...
      if (files.image.size > 0 && files.image.mimetype.includes("image")) {
        ...
        newPost = new Post({
          ...
          hashtag: fields.body
            ...
            .split(" ")
            .filter(
              (item) => item.includes("#") && item.trim() && item.length > 1
            ),
        });
      } else {
        res.error(new ErrorHandler("Please Enter a Valid Image File", 400));
...
exports.commentDelete = AsyncError(async (req, res, next) => {
  ...
  post.comments.splice(
    post.comments.findIndex((item) => item._id.toString() === commentID),
    1
  );
  post.save();
  res.redirect(`back`);
